package org.battleplugins.api.sponge.compat;
 ...
 */
public enum SpongeCompatItemType {
    ...
    private static Map<String, SpongeCompatItemType> cachedSearch = new HashMap<>();

    private String[] materials;
    private int data;
    private ItemType cached = null;

    SpongeCompatItemType(int data, String... materials) {
    ...
    public ItemStack parseItem() {
        ItemType mat = parseMaterial();
        if (isNewVersion())
            return ItemStack.builder().itemType(mat).build();

        ItemStack stack = ItemStack.builder().itemType(mat).quantity(1).build();
        return ItemStack.builder()
                .fromContainer(stack.toContainer().set(DataQuery.of("UnsafeDamage"), data))
                .build();
    }
    ...
    public ItemType parseMaterial() {
        if (cached != null)
            return cached;

        Optional<ItemType> mat = Sponge.getRegistry().getType(ItemType.class, this.toString());
        return mat.map(itemType -> cached = itemType).orElseGet(() -> cached = Sponge.getRegistry().getType(ItemType.class, materials[0]).orElse(ItemTypes.AIR));
    }
    ...
    public static Optional<SpongeCompatItemType> fromString(String key) {
        ...
        SpongeCompatItemType mat;

        try {
            mat = SpongeCompatItemType.valueOf(matKey);
            return Optional.of(mat);
        } catch (IllegalArgumentException ex) {
            String[] split = matKey.split(":");
            if (split.length == 1)
                mat = requestSpongeItem(matKey,  0).orElse(null);
            else
                mat = requestSpongeItem(split[0], Integer.parseInt(split[1])).orElse(null);

            return Optional.ofNullable(mat);
        }
    }

    public static Optional<SpongeCompatItemType> requestSpongeItem(String name, int data) {
        if (cachedSearch.containsKey(name.toUpperCase() + "," + data))
            return Optional.of(cachedSearch.get(name.toUpperCase() + "," + data));

        for (SpongeCompatItemType mat : SpongeCompatItemType.values()) {
    ...
    public static boolean isNewVersion() {
        return Sponge.getRegistry().getType(ItemType.class, "RED_WOOL").isPresent();
    }
}